...
- **askill adapter:** ${report.acquisition?.askill?.listed ? `1 / 1 exact skill listed at ${report.acquisition.askill.listing_url}; ${Number(report.acquisition.askill.favorites || 0)} public favorites` : report.acquisition?.askill?.status || "awaiting audited catalog refresh"}; exact install source ${report.acquisition?.askill?.install_source || "gh:cristianmoroaica/bountyverdict-mcp-skill@route-github-agent-decisions"} (listing and favorites are not impressions, installs, tool calls, purchases, or revenue; askill exposes no public install counter)
...
- askill adapter: ${report.acquisition?.askill?.listed_skills ?? 0} / 1 exact skill listed; ${report.acquisition?.askill?.status || "unavailable"}; ${Number(report.acquisition?.askill?.favorites || 0)} public favorites (${report.acquisition?.askill?.url || "https://askill.sh/skills/703721"}; catalog presence and favorites are not impressions, installs, tool calls, purchases, or revenue)
