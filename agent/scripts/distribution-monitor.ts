...
- **Smithery publication:** publish-ready; SmitheryBot successfully initialized the existing public Streamable HTTP endpoint and listed all six tools. One-time publisher authentication and namespace ownership remain; keep credentials outside Git and add Smithery's fallback server card only if the real publication scan fails.
...
2. Complete the one-time Smithery publisher login, claim or select the matching owner namespace, and publish the existing remote endpoint without changing its tools or prices. Treat catalog presence as distribution only; if the live scan fails, diagnose it before considering Smithery's optional static server-card fallback.
