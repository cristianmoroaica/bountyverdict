...
- **MCP Observatory:** ${report.acquisition?.mcp_observatory?.status || "unavailable"} (${report.acquisition?.mcp_observatory?.endpoint_exposed ? "endpoint exposed" : "repository metadata only"}; automatic indexing is never an impression, tool call, purchase, or revenue)
